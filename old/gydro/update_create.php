...
$arSettings["MODULE_ID"] = 'hellodigital.turbosite';
        ...
        <table>
            <tr>
                <td valign="top">
                    ...
                    <input type="checkbox" name="UPD[REMOVE_UPD]" value="Y" style="display: inline-block; margin-right: 10px;"> Очистить директорию сборки<br/>
                    <input type="checkbox" name="UPD[ARCHIVE]" value="Y" style="display: inline-block; margin-right: 10px;"> Создать архив<br/>
                </td>
                <td align="left" valign="top" style="padding-left:25px;">
            <?foreach($arResult as $path=>$arFile):?>
                <input type="checkbox" value="<?=substr($path, $l)?>" name="UPD[FOLDERS][]" style="display: inline-block; margin-right: 10px;"><b><?=substr($path, $l)?></b><br/>
                <?foreach($arFile as $file):?>
                    <input type="checkbox" value="<?=substr($path, $l).$file?>" name="UPD[FILES][]" style="display: inline-block; margin-right: 10px;"><small><i><?=$file?></i></small><br/>
                <?endforeach;?>
            <?endforeach;?>
                </td>
            </tr>
